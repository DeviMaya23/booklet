...
An authenticated user SHALL be able to retrieve all artists they own, ordered by name ascending.
...
An authenticated user SHALL be able to partially update an artist they own using PATCH semantics. Only fields present in the request body SHALL be updated; absent fields SHALL remain unchanged.

Updatable fields: `name`, `notes`, `artist_link`.

#### Scenario: Successful update
- **WHEN** an authenticated user sends `PATCH /artists/:id` with a subset of fields
- **THEN** the system returns 200 with the updated artist, and omitted fields retain their previous values

#### Scenario: Duplicate name on update
- **WHEN** an authenticated user sends `PATCH /artists/:id` with a name already used by another artist they own
- **THEN** the system returns 409

#### Scenario: Artist not found or not owned
- **WHEN** an authenticated user sends `PATCH /artists/:id` for an artist that does not exist or belongs to another user
- **THEN** the system returns 404

#### Scenario: Invalid artist_link format
- **WHEN** an authenticated user sends `PATCH /artists/:id` with `artist_link` set to a non-URL string
- **THEN** the system returns 422

#### Scenario: Malformed request body
- **WHEN** an authenticated user sends `PATCH /artists/:id` with invalid JSON
- **THEN** the system returns 400
