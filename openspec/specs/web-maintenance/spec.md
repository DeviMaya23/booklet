...
The maintenance capability governs how the Booklet web application detects, displays, and exits maintenance mode. It relies on API response headers to signal state changes and uses a localStorage bypass token to allow privileged access during active maintenance.
